...
function sanitizePersistedFeishuChannel(channel = {}) {
  ...
  const next = {};
  const appId = String(channel.appId ?? channel.credentials?.appId ?? channel.settings?.appId ?? '').trim();
  const appSecret = String(channel.appSecret ?? channel.credentials?.appSecret ?? channel.settings?.appSecret ?? '').trim();
  const verificationToken = String(channel.verificationToken ?? channel.credentials?.verificationToken ?? channel.settings?.verificationToken ?? '').trim();
  const streaming = channel.streaming ?? channel.settings?.streaming;

  if (channel.enabled !== undefined) next.enabled = !!channel.enabled;
  if (appId) next.appId = appId;
  if (appSecret) next.appSecret = appSecret;
  if (verificationToken) next.verificationToken = verificationToken;
  if (streaming !== undefined) next.streaming = streaming;
  if (channel.accounts && typeof channel.accounts === 'object' && !Array.isArray(channel.accounts)) next.accounts = cloneJson(channel.accounts);
  if (channel.tools && typeof channel.tools === 'object' && !Array.isArray(channel.tools)) next.tools = cloneJson(channel.tools);
...
function sanitizePersistedConfigShape(config = {}) {
  ...
  if (next.channels && typeof next.channels === 'object' && !Array.isArray(next.channels)) {
    ...
    if (currentFeishu && typeof currentFeishu === 'object' && !Array.isArray(currentFeishu)) {
...
function upsertChannelConfig(channelKey, payload = {}) {
  ...
  config.channels[key] = key === 'feishu'
    ? sanitizePersistedFeishuChannel({
        ...
        verificationToken: nextCredentials.verificationToken || current.verificationToken || '',
